...
class _PaymentState extends State<Payment> {
  ...
  void initState() {
    ...
    paymentBloc.add(LoadPaymentDataEvent(
        babysisterId: widget.babysisterId, shiftId: widget.shiftId));
    WidgetsBinding.instance.addPostFrameCallback((_) {
      paymentBloc.stream.listen((state) {
        if (state.paymentStateStatus == PaymentStateStatus.success ||
            state.paymentStateStatus == PaymentStateStatus.updatedItem) {
          controllers.clear();
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                        ...
                        GestureDetector(
                          ...
                          child: Text(
                            DateFormat('yyyy - MM').format(DateTime(
                                paymentState.year, paymentState.month)),
                    ...
                    Visibility(
                        visible: paymentState.paymentStateStatus !=
                            PaymentStateStatus.emptyPayment,
                        child: Column(
                          children: [
                            Padding(
                                ...
                                children: [
                                  Expanded(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        '${'hourlyWage'.tr()}: ${paymentState.payment?.hourlyWage ?? 0}',
                                        style: const TextStyle(
                                      ...
                                      Text(
                                        '${paymentState.payment?.totalDays ?? 0} ${'days'.tr()} - ${paymentState.payment?.hourWorking ?? 0} ${'hours'.tr()}',
                                        style: const TextStyle(
                                          color: CustomColors.gray,
                                          fontSize: 14,
                                          fontFamily: "Roboto",
                                  ...
                                  Text(
                                    NumberFormat('###,###').format(
                                        paymentState.payment?.labourCost ?? 0),
                                    style: const TextStyle(
                                      color: Colors.black,
                                      fontSize: 14,
                                      fontFamily: "Roboto",
                            ...
                            Column(
                              children: (paymentState.payment?.items ?? [])
                                  .map((item) => Padding(
        ...
        child: SafeArea(
          ...
          child: CupertinoDatePicker(
            ...
            onDateTimeChanged: (DateTime newDate) {
              paymentBloc.add(
                  ChangeMonthEvent(shiftId: widget.shiftId, dateTime: newDate));
            },
          ),
        ),
      ),
            ...
            Row(
              ...
              children: [
                CustomRadiusCheckbox(
                  ...
                  onChanged: (value) {
                    paymentBloc
                        .add(IncludeInPaymentEvent(index: index, value: value));
                  },
                ),
                const SizedBox(
                  width: 10.0,
        ...
        item.canRemove
            ? SizedBox(
                ...
                child: CustomInputField(
                  ...
                  onChanged: (value) {
                    paymentBloc.add(ChangePriceEvent(
                        index: index,
