...
class CostListState {
  ...
  double totalCost() {
    ...
    totalCost = (cost?.labourCost ?? 0) +
        ((cost?.hourlyWave ?? 0) * (cost?.hourWorking ?? 0)) +
        totalCost;
    return totalCost;
  }
  ...
  List<dynamic> getOnlyOldImage() {
    ...
    if (cost?.picture1 != null)
      images.add(convertBase64Image(cost?.picture1 ?? ''));
    if (cost?.picture2 != null)
      images.add(convertBase64Image(cost?.picture2 ?? ''));
    if (cost?.picture3 != null)
      images.add(convertBase64Image(cost?.picture3 ?? ''));
