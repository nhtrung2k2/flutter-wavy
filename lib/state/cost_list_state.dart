...
class CostListState {
  ...
  int totalCost(){
    ...
    totalCost = (cost?.labourCost ?? 0) + (cost?.amount ?? 0) + totalCost;
    return totalCost;
  }
  ...
  List<dynamic> getOnlyOldImage(){
    ...
    if(cost?.picture1!=null) images.add(base64Decode(cost?.picture1 ?? ''));
    if(cost?.picture2!=null) images.add(base64Decode(cost?.picture2 ?? ''));
    if(cost?.picture3!=null) images.add(base64Decode(cost?.picture3 ?? ''));
