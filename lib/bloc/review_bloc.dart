...
class ReviewBloc extends Bloc<ReviewEvent, ReviewState> {
  ...
  Future<void> _onInitData(
    ...
    try {
      Employee_Detail employeeDetail = await _employeesRepository
          .fetchEmployDetail(event.babysistterId, event.shiftId);
      emit(state.copyWith(
  ...
  Future<void> _onSwitchToggle(
    ...
    if (event.index == 0) {
      emit(state.copyWith(isBabysittingProvided: event.value));
    } else if (event.index == 1) {
      emit(state.copyWith(isCookingProvided: event.value));
    } else if (event.index == 2) {
      emit(state.copyWith(isLaundryProvided: event.value));
    } else if (event.index == 3) {
      emit(state.copyWith(isCleaningProvided: event.value));
    }
  }
  ...
  Future<void> _onDisplayed(
    ...
    emit(state.copyWith(
        review: state.review.copyWith(displayed: event.value ? 1 : 0)));
  }
  ...
  Future<void> _onChangeRating(
    ...
    if (event.runtimeType == ChangeOverallRateEvent) {
      _onResetValidate(
          ResetValidate(status: ReviewValidateStatus.emptyOverallRatting),
          emit);
      emit(state.copyWith(
          review: state.review.copyWith(overallRating: event.rate)));
    } else if (event.runtimeType == ChangeCommunicationRateEvent) {
      _onResetValidate(
          ResetValidate(status: ReviewValidateStatus.emptyCommunicationRatting),
          emit);
      emit(state.copyWith(
          review: state.review.copyWith(communicationRating: event.rate)));
    } else if (event.runtimeType == ChangeAttCleanRateEvent) {
      _onResetValidate(
          ResetValidate(status: ReviewValidateStatus.emptyAttCleanRatting),
          emit);
      emit(state.copyWith(
          review:
              state.review.copyWith(attitudeCleanlinessRating: event.rate)));
      ...
      emit(state.copyWith(
          review: state.review.copyWith(cookingRating: event.rate)));
  ...
  Future<void> _onResetValidate(
    ...
    emit(state.copyWith(
        reviewStateStatus: ReviewStateStatus.submitting, validateStatus: list));
  }
  ...
  Future<void> _onSubmit(
    ...
    emit(state.copyWith(
        reviewStateStatus: ReviewStateStatus.submitting, validateStatus: []));
    ...
    if (validateStatus.isEmpty) {
      try {
        Response response = await _reviewRepository.createReview(
            review.copyWith(
              dateStart: DateFormat('yyyy-MM-dd')
                  .format(DateFormat('yyyy-MM-dd').parse(review.dateStart!)),
              dateEnd: DateFormat('yyyy-MM-dd')
                  .format(DateFormat('yyyy-MM-dd').parse(review.dateEnd!)),
            ),
            event.babysistterId);
      ...
      emit(state.copyWith(validateStatus: validateStatus));
    }
  }
}
